...
CREATE TABLE adverse_events(
    PRIMARY KEY(adverse_events_id),
    adverse_events_id TINYINT(1) UNSIGNED AUTO_INCREMENT,
...
CREATE TABLE adverse_events_log(
    ...
    adverse_events_id TINYINT(1) UNSIGNED,
    event_date DATE,
    FOREIGN KEY(visit_id) REFERENCES visits(visit_id) ON UPDATE CASCADE,
    FOREIGN KEY(adverse_events_id) REFERENCES adverse_events(adverse_events_id) ON UPDATE CASCADE
    );
    
CREATE UNIQUE INDEX unique_index_ael
    ON adverse_events_log(visit_id, adverse_events_id, event_date);

CREATE INDEX adverse_events_id
    ON adverse_events_log(adverse_events_id);
    
CREATE INDEX event_date
    ON adverse_events_log(event_date); 
